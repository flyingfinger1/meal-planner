...
// In-memory share tokens (token -> { dates, expiresAt })
const shareTokens = new Map<string, { dates: string[]; expiresAt: number }>();
...
router.post('/shopping/share-token', (req, res) => {
...
router.get('/recipe/shopping/:token', (req, res) => {
  const tokenData = shareTokens.get(req.params.token);
  if (!tokenData || tokenData.expiresAt < Date.now()) {
    return res.status(404).send('Link abgelaufen');
  }

  const { dates } = tokenData;
  const placeholders = dates.map(() => '?').join(',');
  const ingredients = queryAll(`
    SELECT i.name, i.amount, i.category, m.name as meal_name
    FROM meal_plan mp
...
router.get('/recipe/quick-list/:id', (req, res) => {
...
router.get('/recipe/:mealId', (req, res) => {
